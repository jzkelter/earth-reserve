...
globals[
  ...
  ;; ^ can keep this if want to keep track of average asset in market value
  DATE ;DATE is the current date of the model, it moves forward 1 day for every tic
  ;; ^ need to change this to be in months
  ...
  ;; ^ can keep this if want to keep track of average ownership length per financial athlete
]

turtles-own[
  personal-assets ;personal-assets is a list of all assets a particular turtle owns
  ;; ^ equivalent to deposit-receipts
  ...
  ;; ^ this would just be checking for valuation
  income ;income represents the amount of income a turtle will receive at constant intervals
  ;; ^ no need this, aiv from ending of projects will be in its place
]

;; TAX SYSTEM NEEDS TO CHANGE, because they don't get paid simultaneously with the "tax bill"
;; but instead on the anniversary of acquiring the DR
...
to setup ;the observer runs this proceudre
  ...
  ask turtles [
    ...
    ;; ^ not necessary, as income will be from aiv redemption
  ]
  reset-ticks
end
